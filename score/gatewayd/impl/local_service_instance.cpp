...
Result<std::unique_ptr<LocalServiceInstance>> LocalServiceInstance::Create(
    ...
    for (auto event_config : *instance->service_type_config_->events()) {
        ...
        if (const auto set_handler_result = ipc_event.SetReceiveHandler([instance_ptr =
                                                                             instance.get(),
                                                                         &ipc_event,
                                                                         &event_context]() {
                if (const auto get_samples_result = ipc_event.GetNewSamples(
                        [instance_ptr, &event_context](SamplePtr<void> sample) {
                            auto maybe_payload =
                                instance_ptr->server_connector_->allocate_event_payload(
                                    event_context.socom_event_id);
                            if (!maybe_payload.has_value()) {
                                std::cout
                                    << "[gatewayd] LocalServiceInstance - Failed to allocate event "
                                       "payload for event "
                                    << event_context.socom_event_id << ": "
                            ...
                            std::uint16_t service_id =
                                instance_ptr->service_type_config_->service_id();
                    ...
                    score::mw::log::LogError()
                        ...
                        << event_context.socom_event_id << ": "
                        << get_samples_result.error().Message();
                }
            });
            !set_handler_result) {
            score::mw::log::LogError()
