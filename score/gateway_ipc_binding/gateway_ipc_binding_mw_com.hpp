...
///
/// \note    Implementation status: only peer liveness is implemented so far. Creating a server
///          provides the `SomeipdService`, creating a client consumes it and reports it through
///          `is_connected()`. Bridged services passed in `Service_configs` are validated and stored
///          but not yet set up; see `create_client()` / `create_server()`.
...
/// \note    Bridged services are not set up yet, see the namespace documentation. A non-empty
///          `services` argument is accepted and logged, but has no effect.
...
/// \return Nullptr if service discovery for SomeipdService could not be started
...
///         mw::com is asked to create the instance, such as a specifier missing from
