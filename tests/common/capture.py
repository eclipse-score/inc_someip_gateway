...
def stop_capture(
    ...
    """Stop *proc* (tcpdump), preferring a signal-free natural exit, falling
    back to SIGINT then SIGKILL on timeout.

    Natural close: closing our end of tcpdump's stdout pipe makes its next
    write raise SIGPIPE (default action: terminate), without us ever sending
    a signal. This sidesteps the -Z privilege-drop sandbox barrier where
    ``send_signal``/``killpg`` raise EPERM. Only effective in text mode
    (stdout=PIPE); pcap-file mode uses DEVNULL and falls through to SIGINT.

    ``pkill -9 -x tcpdump`` always runs before returning (except when the
    process had already exited before this call) to mop up any
    privilege-separation child processes forked by tcpdump's -Z handling.
    Such children are not tracked by the Popen object and can survive even a
    clean exit of the parent, so the sweep cannot be limited to the
    timeout/SIGKILL branch.

    Returns True if the process exited cleanly (or was already gone), False if
    SIGKILL was required or a PermissionError was hit.
    """
    ...
    if proc.stdout is not None:
        ...
        try:
            ...
            pass  # stdout close did not trigger exit; fall back to signals
...
def tcpdump_capture(
    ...
    try:
        ...
        raise RuntimeError(f"tcpdump failed to start: {e}. Check that /usr/bin/tcpdump exists and is executable.") from e
