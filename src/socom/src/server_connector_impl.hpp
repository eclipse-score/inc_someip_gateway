...
public:
    ...
    // \retval false None (client not contained) or one of more than 2 existing clients has been removed
...
public:
    ...
    Enabled_server_connector::Result<None> update_event(Event_id server_id,
                                                        Payload::Sptr payload) noexcept override;
    Enabled_server_connector::Result<None>
    update_requested_event(Event_id server_id, Payload::Sptr payload) noexcept override;
    Enabled_server_connector::Result<None>
    set_event_subscription_state(Event_id server_id, Event_state event_state) noexcept override;
    Enabled_server_connector::Result<Event_mode>
    get_event_mode(Event_id server_id) const noexcept override;
...
private:
    ...
    template<typename MessageType>
    static typename MessageType::Return_type
    send(std::vector<Client_connector_endpoint> const& clients, MessageType message);
