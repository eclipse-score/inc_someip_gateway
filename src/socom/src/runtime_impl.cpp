...
std::thread::id get_invalid_thread_id()
{
    return {};
}
...
template<typename T>
void cleanup(std::list<std::weak_ptr<T const>>& list, std::shared_ptr<T const> const& item) noexcept
{
...
bool is_valid(Client_connector::Callbacks const& callbacks)
{
    return callbacks.on_service_state_change && callbacks.on_event_update &&
           callbacks.on_event_requested_update && callbacks.on_event_subscription_status_change;
}
...
get_bridge_requests(Service_interface_configuration const& configuration, Instance const& instance,
                    ...
                    CreateValue const& create_value)
{
    ...
    // NOLINTNEXTLINE(performance-unnecessary-copy-initialization) copy is needed to unlock mutexes and modify bridge_to_callback
    ...
    for (auto& bridge_id_callback_result : tmp_result) {
        ...
        // TODO(lure9428): This case should be further discussed with lure9428.
...
Service_database::get_instances(Service_interface const& interface,
...
Interfaces_instances
Service_database::get_instances(std::optional<Service_interface> const& interface,
                                std::optional<Service_instance> const& filter) const
{
    if (interface) {
...
Service_record::Server_registration
Service_record::register_server_connector(Service_interface const& interface,
...
Service_record::Client_registration
Service_record::register_client_connector(Service_interface const& interface,
...
Runtime::Result<Client_connector::Uptr>
Runtime_impl::make_client_connector(Service_interface_configuration const& configuration,
                                    Service_instance const& instance,
    ...
    Client_connector::Callbacks callbacks, Posix_credentials const& credentials) noexcept
{
    if (!is_valid(callbacks)) {
        return Runtime::Result<Client_connector::Uptr>::unexpected_type{
            Construction_error::callback_missing};
    }
...
Runtime::Result<Disabled_server_connector::Uptr>
Runtime_impl::make_server_connector(Server_service_interface_configuration const& configuration,
                                    Service_instance const& instance,
    ...
    Disabled_server_connector::Callbacks callbacks, Posix_credentials const& credentials) noexcept
{
    ...
    if (!is_valid(callbacks)) {
        return Runtime::Result<Disabled_server_connector::Uptr>::unexpected_type{
            Construction_error::callback_missing};
    }
...
Find_aggregation::Find_aggregation(Find_result_callback cb) : m_on_result_set_change{std::move(cb)}
{
}
...
public:
    ...
    Find_aggregation_subscription_handle&
    operator=(Find_aggregation_subscription_handle const& other) = delete;
    Find_aggregation_subscription_handle&
    operator=(Find_aggregation_subscription_handle&& other) = delete;
...
Find_subscription
Runtime_impl::subscribe_find_service(Find_result_callback on_result_set_change,
...
Runtime::Result<Service_bridge_registration>
Runtime_impl::register_service_bridge(Bridge_identity identity,
                                      Subscribe_find_service_function subscribe_find_service,
                                      Request_service_function request_service) noexcept
{
    if (!subscribe_find_service || !request_service) {
        return Runtime::Result<Service_bridge_registration>::unexpected_type{
            Construction_error::callback_missing};
    }
                                              ...
                                              CC_impl::Server_indication const& on_server_update)
{
    ...
    Registration bridged_service_requests; // LCOV_EXCL_LINE, lcov bug
...
// NOLINTNEXTLINE(bugprone-exception-escape)(ClangTidy Android Warning)
void Runtime_impl::stop_subscription(Find_subscription_id const& id) noexcept
{
    ...
    // NOLINTNEXTLINE(performance-unnecessary-copy-initialization) copy is needed because memory will be deleted
...
Runtime_impl::create_bridge_find_result_callback(Bridge_registration_id const& bridge_id)
{
...
std::shared_ptr<Bridge_id_to_subscription>
Runtime_impl::get_or_create_find_services(Service_interface const& interface,
                                          std::optional<Service_instance> const& instance,
                                          std::optional<Bridge_identity> identity)
{
    auto const create_value = [this](auto const& callbacks, auto const& configuration,
                                     auto const& instance) {
        auto const notify_find_subscriptions = // LCOV_EXCL_LINE, lcov bug
...
Registration
Runtime_impl::bridge_service_requests(Service_interface_configuration const& configuration,
                                      Service_instance const& instance)
{
...
std::shared_ptr<Bridge_id_to_request>
Runtime_impl::get_or_create_service_requests(Service_interface_configuration const& configuration,
                                             Service_instance const& instance)
{
...
Interfaces_instances
Runtime_impl::get_bridge_reported_instances(Service_interface const& interface,
