...
private:
    ...
    get_or_create_service_requests(Service_interface_configuration const& configuration,
                                   Service_instance const& instance);
    ...
    create_bridge_find_result_callback(Bridge_registration_id const& bridge_id);

    std::shared_ptr<Bridge_id_to_subscription>
    get_or_create_find_services(Service_interface const& interface,
                                std::optional<Service_instance> const& instance,
                                std::optional<Bridge_identity> identity);

    Interfaces_instances
    get_bridge_reported_instances(Service_interface const& interface,
