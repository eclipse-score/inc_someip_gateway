...
Service_instance const& Impl::get_service_instance() const noexcept
{
    return m_instance;
}

Enabled_server_connector::Result<None> Impl::update_event(Event_id server_id,
                                                          Payload::Sptr payload) noexcept
{
    if (server_id >= m_configuration.get_num_events()) {
        return create_error<None>();
    }
    ...
    return Enabled_server_connector::Result<None>{None{}};
}

Enabled_server_connector::Result<None> Impl::update_requested_event(Event_id server_id,
                                                                    Payload::Sptr payload) noexcept
{
    if (server_id >= m_configuration.get_num_events()) {
        return create_error<None>();
...
Enabled_server_connector::Result<None>
Impl::set_event_subscription_state(Event_id server_id, Event_state event_state) noexcept
{
    if (server_id >= m_configuration.get_num_events()) {
        return create_error<None>();
    }
...
Enabled_server_connector::Result<Event_mode> Impl::get_event_mode(Event_id server_id) const noexcept
{
    if (server_id >= m_configuration.get_num_events()) {
...
message::Posix_credentials::Return_type
Impl::receive(Client_connection const& /*client*/, message::Posix_credentials const& /* message */)
{
                                                    ...
                                                    message::Subscribe_event message)
{
    ...
    return message::Subscribe_event::Return_type{None{}};
}
                                                      ...
                                                      message::Unsubscribe_event message)
{
    ...
    return message::Unsubscribe_event::Return_type{None{}};
}
                                                         ...
                                                         message::Request_event_update message)
{
    ...
    return message::Request_event_update::Return_type{None{}};
}

} // namespace server_connector

std::unique_ptr<Enabled_server_connector>
Disabled_server_connector::enable(std::unique_ptr<Disabled_server_connector> connector)
{
...
std::unique_ptr<Disabled_server_connector>
Enabled_server_connector::disable(std::unique_ptr<Enabled_server_connector> connector) noexcept
{
