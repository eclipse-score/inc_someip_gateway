...
struct Connect {
    using Return_type = Result<message::Connect_return, Error>;
    Client_connector_endpoint& endpoint;
};

struct Call_method {
    using Return_type = Result<Method_invocation::Uptr, Error>;
    Method_id const id;
...
struct Posix_credentials {
    using Return_type = Result<::score::socom::Posix_credentials, Error>;
};

struct Subscribe_event {
    using Return_type = Result<None, Error>;
    Event_id const id;
