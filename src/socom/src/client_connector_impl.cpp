...
Impl::Impl(Runtime_impl& runtime, Service_interface_configuration const& configuration,
    ...
    , m_stop_block_token{std::make_shared<Final_action>(
          [this]() { m_stop_complete_promise.set_value(); })}
    , m_registration{runtime.register_connector(configuration, instance,
...
message::Request_event_update::Return_type
Impl::request_event_update(Event_id client_id) const noexcept
{
    return send(message::Request_event_update{client_id});
}

message::Call_method::Return_type
Impl::call_method(Method_id client_id, Payload::Sptr payload,
