...
Clients subscribe to an event.
On the first subscription, SOCom informs the server using the request for events.
All further subscriptions are not indicated to the server anymore.
The server is decoupled from the particular subscribers and it is informed only about the fact that at least one client is subscribed to the event.
If no client is subscribed to an event, the server will not send any event updates.

SOCom takes care of event fan-out in case of a server event update.
After unsubscription of the last subscriber, SOCom informs the server that an event is not subscribed anymore.
...
Those updates are indicated to clients which subscribed and have an initial event update pending.

Regular update_event() calls update for the events of all subscribers and also fulfills the initial event update use-case.
