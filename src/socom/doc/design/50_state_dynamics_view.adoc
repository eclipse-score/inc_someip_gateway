...
| Event_subscribed_acknowledged
| The user is subscribed to the event.
The server has acknowledged the reception of the event subscription and will send event updates.
Clients know that the server will send event updates.
|====
...
| Client_connector::Callbacks::on_event_subscription_status_change(Event_state::subscribed)
| SOCom calls the user callback Client_connector::Callbacks::on_event_subscription_status_change(Event_state::subscribed) in order to indicate the successful subscription to an event.

| Client_connector::Callbacks::on_event_subscription_status_change(Event_state::unsubscribed)
| SOCom calls the user callback Client_connector::Callbacks::on_event_subscription_status_change(Event_state::unsubscribed) in order to indicate the loss of an event subscription.
