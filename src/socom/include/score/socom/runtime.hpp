...
public:
    Find_subscription_handle() = default;          ///< Default constructor.
    virtual ~Find_subscription_handle() = default; ///< Default destructor.

    Find_subscription_handle(Find_subscription_handle const&) =
        delete; ///< Copy constructor cannot be used.
    Find_subscription_handle(Find_subscription_handle&&) =
        delete; ///< Move constructor cannot be used.

    Find_subscription_handle& operator=(Find_subscription_handle const&) =
        delete; ///< Copy assignment operator cannot be used.
    Find_subscription_handle&
    operator=(Find_subscription_handle&&) = delete; ///< Move assignment operator cannot be used.
};

/// \brief Interface class for Service_bridge_registration RAII type (see Runtime).
class Service_bridge_registration_handle {
public:
    Service_bridge_registration_handle() = default;          ///< Default constructor.
    virtual ~Service_bridge_registration_handle() = default; ///< Default destructor.

    Service_bridge_registration_handle(Service_bridge_registration_handle const&) =
        delete; ///< Copy constructor cannot be used.
    Service_bridge_registration_handle(Service_bridge_registration_handle&&) =
        delete; ///< Move constructor cannot be used.

    Service_bridge_registration_handle& operator=(Service_bridge_registration_handle const&) =
        delete; ///< Copy assignment operator cannot be used.
    Service_bridge_registration_handle& operator=(Service_bridge_registration_handle&&) =
        delete; ///< Move assignment operator cannot be used.
...
public:
    Service_request_handle() = default;          ///< Default constructor.
    virtual ~Service_request_handle() = default; ///< Default destructor.

    Service_request_handle(Service_request_handle const&) =
        delete;                                                ///< Copy constructor cannot be used.
    Service_request_handle(Service_request_handle&&) = delete; ///< Move constructor cannot be used.

    Service_request_handle&
    operator=(Service_request_handle const&) = delete; ///< Copy assignment operator cannot be used.
    Service_request_handle&
    operator=(Service_request_handle&&) = delete; ///< Move assignment operator cannot be used.
};
...
public:
    ...
    template<typename T>
    using Result = Result<T, Construction_error>;

    Runtime() = default;                         ///< Default constructor.
    virtual ~Runtime() noexcept = default;       ///< Default destructor.
    Runtime(Runtime const&) = delete;            ///< Copy constructor cannot be used.
    Runtime(Runtime&&) = delete;                 ///< Move constructor cannot be used.
    Runtime& operator=(Runtime const&) = delete; ///< Copy assignment operator cannot be used.
    Runtime& operator=(Runtime&&) = delete;      ///< Move assignment operator cannot be used.
    ...
    [[nodiscard]]
    virtual Result<Client_connector::Uptr>
    make_client_connector(Service_interface_configuration const& configuration,
    ...
    [[nodiscard]]
    virtual Result<Disabled_server_connector::Uptr>
    make_server_connector(Server_service_interface_configuration const& configuration,
                          Service_instance const& instance,
    ...
    [[nodiscard]]
    virtual Find_subscription
    subscribe_find_service(Find_result_callback on_result_set_change,
    ...
    [[nodiscard]]
    virtual Find_subscription
    subscribe_find_service(Find_result_change_callback on_result_change,
                           std::optional<Service_interface> interface,
    ...
    /// \note Construction_error::callback_missing is returned if any of the callbacks is not set.
    [[nodiscard]]
    virtual Result<Service_bridge_registration>
    register_service_bridge(Bridge_identity identity,
