...
public:
    ...
    ///
    /// \attention All user callbacks must not block and shall return quickly (simple algorithms only).
    /// No callback is allowed to destroy the Client_connector, otherwise it will result in a
    ...
    Client_connector(Client_connector const&) = delete; ///< Copy constructor cannot be used.
    Client_connector(Client_connector&&) = delete;      ///< Move constructor cannot be used.

    Client_connector&
    operator=(Client_connector const&) = delete; ///< Copy assignment operator cannot be used.
    Client_connector&
    operator=(Client_connector&&) = delete; ///< Move assignment operator cannot be used.
    ...
    virtual Result<None> subscribe_event(Event_id client_id, Event_mode mode) const noexcept = 0;
    ...
    /// \param client_id ID of the event.
    /// \return Void in case of successful operation, otherwise an error.
    virtual Result<None> unsubscribe_event(Event_id client_id) const noexcept = 0;
    ...
    /// \param client_id ID of the event.
    /// \return Void in case of successful operation, otherwise an error.
    virtual Result<None> request_event_update(Event_id client_id) const noexcept = 0;
    ...
    call_method(Method_id client_id, Payload::Sptr payload,
    ...
    [[nodiscard]] virtual Service_interface_configuration const&
    get_configuration() const noexcept = 0;
