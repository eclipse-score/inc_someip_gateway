...
// copy with small modifications from baselibs to be able to compile socom as shared library
...
using converts_from_any_cvref =
    std::disjunction<std::is_constructible<T, W&>, std::is_convertible<W&, T>,
                     std::is_constructible<T, W>, std::is_convertible<W, T>,
                     std::is_constructible<T, const W&>, std::is_convertible<const W&, T>,
                     std::is_constructible<T, const W>, std::is_convertible<const W, T>>;
// Suppress "AUTOSAR C++14 A12-8-6" rule findings. The rule states "Copy and move constructors and
// copy assignment and move assignment operators shall be declared protected or defined "=delete" in
// base class." Defining  copy and move constructors in base class will make the expected non
// trivial and also the derived classes will be non copyable as stated by the rule
// https://timsong-cpp.github.io/cppwp/n4950/expected#object.cons-10
...
template<typename E>
// We deliberately do not adhere to the rule of five here as both the destructor and the copy
// assignment operators Suppress "AUTOSAR C++14 A12-0-1" rule states: If a class declares a copy or
// move operation, or a destructor, either via “=default”, “=delete”, or via a user-provided
// declaration, then all others of these five special member functions shall be declared as well.".
// We deliberately do not adhere to the rule of five here as both the destructor and the copy
// assignment operators. NOLINTBEGIN(cppcoreguidelines-special-member-functions): shall be
// implicitly declared as per the C++23 standard. coverity[autosar_cpp14_a12_0_1_violation]
class unexpected
    : impl::base_unexpected
// NOLINTEND(cppcoreguidelines-special-member-functions): shall be implicitly declared as per the
// C++23 standard.
{
public:
    constexpr unexpected(const unexpected&) = default;
    // We deliberately do not declare the move constructor noexcept according to the C++23 standard.
    ...
    template<
        ...
        typename = std::enable_if_t<std::conjunction_v<
            ...
            std::negation<
                std::is_same<std::remove_cv_t<std::remove_reference_t<G>>, std::in_place_t>>,
    ...
    // We deliberately do not declare the move assignment operator noexcept according to the C++23
    // standard. In addition, since it is defaulted, it will anyway inherit the noexcept.
    ...
    {
        // Suppress AUTOSAR C++14 A18-9-3" rule violations. The rule states "The std::move shall not
        // be used on objects declared const or const&." the move operation on a const object does
        // not cause unintended behavior it ensures a proper copy/move of underlying values.
    ...
    // Suppress "AUTOSAR C++14 A13-5-5" rule violation". The rule states "Comparison operators shall
    // be non-member functions with identical parameter types and noexcept." The standard does not
    // specify them noexcept and we follow that. Suppress "AUTOSAR C++14 A3-3-1" rule violations.
    // The rule states "Objects or functions with external linkage (including members of named
    // namespaces) shall be declared in a header file". This is false positive, the external linkage
    // function is declared in the header file. coverity[autosar_cpp14_a3_3_1_violation : FALSE]
...
// Suppress "AUTOSAR C++14 A16-0-1" rule findings. This rule stated: "The pre-processor shall only
// be used for unconditional and conditional file inclusion and include guards, and using the
// following directives: (1) #ifndef, #ifdef, (3) #if, (4) #if defined, (5) #elif, (6) #else, (7)
// #define, (8) #endif, (9) #include.". Suppress "AUTOSAR C++14 M16-0-2" rule findings. This rule
// stated: "Macros shall only be #define’d or #undef’d in the global namespace". Tolerated by
// project to enable/disable enforce and expected nodiscard.
...
class SPP_EXPECTED_NODISCARD expected : impl::base_expected {
    // Suppress "AUTOSAR C++14 A5-1-7" rule finding. This rule states: "A lambda shall not be an
    // operand to decltype or typeid". False-positive, at this point "decltype" is not used with
    // lambda. coverity[autosar_cpp14_a5_1_7_violation : FALSE]
...
public:
    ...
    template<typename U, typename G,
             typename = std::enable_if_t<std::conjunction_v<
                 std::is_constructible<T, const U&>, std::is_constructible<E, const G&>,
                 // coverity[autosar_cpp14_a2_11_1_violation] Used intentionally in compliance with
                 // the standard
                 ...
                 std::negation<std::is_constructible<unexpected<E>, const expected<U, G>>>>>>
    // DIVERGENCE FROM STANDARD: Explicitness cannot be specified according to standard, since this
    // requires a C++20 feature. Anyway, only available if T can be implicitly constructed from UF,
    // or E can be implicitly constructed form GF.
    // NOLINTNEXTLINE(google-explicit-constructor) see justification above
    constexpr expected(const expected<U, G>& rhs)
        : impl::base_expected{}
        , storage_{rhs.has_value()
                       ? StorageType{std::in_place_index<0>, std::forward<const U&>(*rhs)}
    ...
    template<typename U, typename G,
             typename = std::enable_if_t<std::conjunction_v<
                 std::is_constructible<T, U>, std::is_constructible<E, G>,
                 // coverity[autosar_cpp14_a2_11_1_violation] Used intentionally in compliance with
                 // the standard
                 std::disjunction<std::is_same<const volatile bool, T>,
                                  std::negation<impl::converts_from_any_cvref<T, expected<U, G>>>>,
    ...
    constexpr expected(expected<U, G>&& rhs)
        : impl::base_expected{}
        , storage_{rhs.has_value()
                       ? StorageType{std::in_place_index<0>, std::forward<U>(*rhs)}
    ...
    template<
        ...
        typename = std::enable_if_t<std::conjunction_v<
            std::negation<
                std::is_same<std::remove_cv_t<std::remove_reference_t<U>>, std::in_place_t>>,
            std::negation<std::is_same<expected, std::remove_cv_t<std::remove_reference_t<U>>>>,
            std::negation<std::is_base_of<impl::base_unexpected,
                                          std::remove_cv_t<std::remove_reference_t<U>>>>,
            std::is_constructible<T, U>,
            std::disjunction<
                // coverity[autosar_cpp14_a2_11_1_violation] Used intentionally in compliance with
                // the standard
                std::negation<std::is_same<const volatile bool,
                                           std::remove_cv_t<std::remove_reference_t<U>>>>,
                std::is_base_of<base_expected, std::remove_cv_t<std::remove_reference_t<U>>>>>>>
    // DIVERGENCE FROM STANDARD: Explicitness cannot be specified according to standard, since this
    // requires a C++20 feature. Anyway, only available if T can be implicitly constructed from U.
    // NOLINTNEXTLINE(google-explicit-constructor) see justification above
    constexpr expected(U&& value)
        : impl::base_expected{}, storage_{std::in_place_index<0>, std::forward<U>(value)}
    {
    }

    template<typename G, typename = std::enable_if_t<std::is_constructible_v<E, const G&>>>
    // DIVERGENCE FROM STANDARD: Explicitness cannot be specified according to standard, since this
    // requires a C++20 feature. Anyway, only available if E can be implicitly constructed from G.
    // NOLINTNEXTLINE(google-explicit-constructor) see justification above
    constexpr expected(const unexpected<G>& error)
        : impl::base_expected{}
        , storage_{std::in_place_index<1>, std::forward<const G&>(error.error())}
    {
    }
    ...
    // Suppress "AUTOSAR C++14 A13-3-1", The rule states: "A function that contains “forwarding
    // reference” as its argument shall not be overloaded". As we here have a different parameters,
    // so no confusion on which API to be used. coverity[autosar_cpp14_a13_3_1_violation]
    ...
    constexpr expected& operator=(expected&& other) noexcept(
        std::conjunction_v<
            std::is_nothrow_move_assignable<T>, std::is_nothrow_move_constructible<T>,
            std::is_nothrow_move_assignable<E>, std::is_nothrow_move_constructible<E>>) = default;
    // NOLINTEND(performance-noexcept-move-constructor)

    template<typename U = T,
             typename = std::enable_if_t<std::conjunction_v<
                 std::negation<
                     std::is_same<expected<T, E>, std::remove_cv_t<std::remove_reference_t<U>>>>,
                 std::negation<std::is_base_of<impl::base_unexpected,
                                               std::remove_cv_t<std::remove_reference_t<U>>>>,
    ...
    {
        // Suppress "AUTOSAR C++14 A0-1-2" rule violatoin. The rule states "The value returned by a
        // function having a non-void return type that is not an overloaded operator shall be used."
        // We are not interested in the return and hence suppressed.
    ...
    // Suppress "AUTOSAR C++14 A15-5-3" rule violation. The rule states "The std::terminate()
    // function shall not be called implicitly". API must be compatible with the standard hence
    // cannot be not noexcept. coverity[autosar_cpp14_a15_5_3_violation]
    ...
    // DIVERGENCE FROM STANDARD: Added [[nodiscard]] for better safety
    [[nodiscard]] constexpr T* operator->() noexcept { return &operator*(); };
    // DIVERGENCE FROM STANDARD: Added [[nodiscard]] for better safety
    [[nodiscard]] constexpr const T* operator->() const noexcept { return &operator*(); };

    // DIVERGENCE FROM STANDARD: Added [[nodiscard]] for better safety
    [[nodiscard]] constexpr bool has_value() const noexcept { return storage_.index() == 0U; }
    // DIVERGENCE FROM STANDARD: Added [[nodiscard]] for better safety
    ...
    // Refer on top for suppression justification.
    // coverity[autosar_cpp14_a15_5_3_violation]
    [[nodiscard]] constexpr E&& error() && noexcept { return std::get<1>(std::move(storage_)); }
    // DIVERGENCE FROM STANDARD: Added [[nodiscard]] for better safety
    // Refer on top for suppression justification.
    // coverity[autosar_cpp14_a15_5_3_violation]
    ...
    {
        using U =
            std::remove_cv_t<std::remove_reference_t<std::invoke_result_t<F, decltype(value())>>>;
    ...
    [[nodiscard]] constexpr auto and_then(F&& fun) &&
    {
        using U = std::remove_cv_t<
            std::remove_reference_t<std::invoke_result_t<F, decltype(std::move(value()))>>>;
    ...
    {
        using G =
            std::remove_cv_t<std::remove_reference_t<std::invoke_result_t<F, decltype(error())>>>;
    ...
    [[nodiscard]] constexpr auto or_else(F&& fun) &&
    {
        using G = std::remove_cv_t<
            std::remove_reference_t<std::invoke_result_t<F, decltype(std::move(error()))>>>;
    ...
    [[nodiscard]] constexpr auto transform(F&& fun) &
    {
        ...
        if (has_value()) {
            // Suppress "AUTOSAR C++14 A7-1-8" rule finding. This rule states: "A class, structure,
            // or enumeration shall not be declared in the definition of its type". This is a false
            // positive because "if constexpr" is a valid statement since C++17.
    ...
    // Suppress "AUTOSAR C++14 A13-5-5", The rule states: "Comparison operators shall be non-member
    // functions with identical parameter types and noexcept.". There is no functional reason
    // behind, we require comparing `expected` and `expected<T2, E2>`. Refer on top for suppression
    // justification. coverity[autosar_cpp14_a13_5_5_violation]
    // coverity[autosar_cpp14_a13_5_5_violation]
    // coverity[autosar_cpp14_a3_3_1_violation : FALSE]
...
// While it's usually UB to modify namespace std, for swap we need to do that so that we can provide
// an overload that is specialized for expected.
namespace std {

template<typename T, typename E>
// Suppress "AUTOSAR C++14 A17-6-1", The rule states: "Non-standard entities shall not be added to
// standard namespaces. While it's usually UB to modify namespace std, for swap we need to do that
// so that we can provide an overload that is specialized for expected. NOLINTBEGIN(cert-dcl58-cpp)
// See comment on the namespace above coverity[autosar_cpp14_a17_6_1_violation]
constexpr void swap(::score::socom::expected<T, E>& lhs, ::score::socom::expected<T, E>& rhs)
...
namespace score::socom {
template<typename T, typename E>
using Result = score::socom::expected<T, E>;
