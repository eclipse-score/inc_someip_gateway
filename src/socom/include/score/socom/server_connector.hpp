...
using Method_call_credentials_callback =
    std::function<Method_invocation::Uptr(Enabled_server_connector&, Method_id, Payload::Sptr,
                                          Method_reply_callback const&, Posix_credentials const&)>;
...
public:
    ...
    Disabled_server_connector() = default;                   ///< Default constructor.
    virtual ~Disabled_server_connector() noexcept = default; ///< Default destructor.

    Disabled_server_connector(Disabled_server_connector const&) =
        delete; ///< Copy constructor cannot be used.
    Disabled_server_connector(Disabled_server_connector&&) =
        delete; ///< Move constructor cannot be used.

    Disabled_server_connector& operator=(Disabled_server_connector const&) =
        delete; ///< Copy assignment operator cannot be used.
    Disabled_server_connector&
    operator=(Disabled_server_connector&&) = delete; ///< Move assignment operator cannot be used.
    ...
    [[nodiscard]]
    static std::unique_ptr<Enabled_server_connector>
    enable(std::unique_ptr<Disabled_server_connector> connector);
...
public:
    ...
    template<typename T>
    using Result = Result<T, Server_connector_error>;
    ...
    Enabled_server_connector(Enabled_server_connector const&) =
        delete; ///< Copy constructor cannot be used.
    Enabled_server_connector(Enabled_server_connector&&) =
        delete; ///< Move constructor cannot be used.

    Enabled_server_connector& operator=(Enabled_server_connector const&) =
        delete; ///< Copy assignment operator cannot be used.
    Enabled_server_connector&
    operator=(Enabled_server_connector&&) = delete; ///< Move assignment operator cannot be used.
    ...
    [[nodiscard]]
    static std::unique_ptr<Disabled_server_connector>
    disable(std::unique_ptr<Enabled_server_connector> connector) noexcept;
    ...
    virtual Result<None> update_event(Event_id server_id, Payload::Sptr payload) noexcept = 0;
    ...
    /// \param payload Event data.
    /// \return Void in case of successful operation, otherwise an error.
    virtual Result<None> update_requested_event(Event_id server_id,
    ...
    /// \return Void in case of successful operation, otherwise an error.
    virtual Result<None> set_event_subscription_state(Event_id server_id,
